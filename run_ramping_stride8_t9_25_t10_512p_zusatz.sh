#!/bin/bash
#SBATCH -J mhd_ramp_stride8_t9_25_t10_512p_zusatz
#SBATCH -p standard
#SBATCH --nodes=1
...
# ============================================================================
# noAC / t370 variant of run_ramping_512p_stride8_t363_t400_LR150_zusatz.sh.
#
...
# without the AC compute overhead.
#
# The dataset window is shifted to t370..401 (to match the coarse4/coarse8
# t370_t401 runs); otherwise unchanged. (Activation checkpointing, found
# inefficient here, has since been removed from the codebase entirely.)
#
# Runs on the ENLARGED dataset finished_512_zusatz (38 train / 11 val sims
# instead of 23 / 6; the old 6 val sims were renamed so they still sort into
# the val tail). The 23-sim run overfit, so the dataset was enlarged and the
# same schedule is replayed here for a directly comparable run.
#
...
# the schedule is rescaled by 0.6 (~= 23/38; total samples seen 1200*38 = 45600
# vs 2000*23 = 46000, within 1%): phase lengths x0.6 and per-epoch increments
# x5/3, plus LR milestones / ckpt_freq rescaled in the config.
#
# That rescaled physics-loss ramp (formerly scripted here as a chain of
# per-phase training runs) now lives entirely in the config's
# `loss_params.weight_schedule`, so this launcher just runs ONE training job.
# See config/stride8_t9_25_t10_512p_zusatz.yaml for the phase table;
# train_params.epochs there is the full run length (1200).
# ============================================================================
...
submit_index_plots() {
    ...
    for v in index_global index_local; do
        ...
        if [ "$EPOCH" = "1200" ] && [ "$v" = "index_global" ]; then
...
# epochs (60 / 480 / 1200) are multiples of ckpt_freq so the checkpoints
# exist; epoch 1200 is the final global-index plot that carries the run email.
# ============================================================================
submit_index_plots 60
submit_index_plots 480
submit_index_plots 1200
submit_b2_plot
