#!/bin/bash
#SBATCH -J mhd_noPDE_stride8_t9_25_t10_512p_zusatz
#SBATCH -p standard
#SBATCH --nodes=1
...
# ============================================================================
# noPDE control of run_ramping_stride8_t9_25_t10_512p_zusatz.sh:
# the same run (enlarged zusatz dataset), but the physics losses are NEVER
# switched on — pure data loss for all 360 epochs (the parent's data-only
# warm-up, continued to the end). 360 = 0.6 x 600, the same rescale the
# parent's schedule uses, so this control is samples-matched to the 600-epoch
# noPDE controls of the 23-sim runs.
#
...
# overrides only the control-defining knobs (one ramp-free 360-epoch
...
#   sbatch launchers_noPDE/run_stride8_t9_25_t10_512p_zusatz_noPDE.sh
# ============================================================================
...
CONFIG_NAME=stride8_t9_25_t10_512p_zusatz_noPDE
PARENT_NAME=stride8_t9_25_t10_512p_zusatz
...
# ============================================================================
# Train: a single run, data loss only for the whole 360 epochs (the overlay's
