...
subroutine write_analysis
  ...
  use mod_metric, only: outerhorizon, BLToCoord
 ...
 character(len=20):: userconvert_type
  ...
  integer, parameter                    :: nqttys = 35, naux=18
  ...
  double precision, dimension(1:nqttys) :: dsum_send, dsum_recv, dmax_send, dmax_recv
  ...
  do iigrid=1,igridstail; igrid=igrids(iigrid);
     ...
     call getcurrent(ixG^LL,ixM^LL,pw(igrid)%w,px(igrid)%x,current,.false.)
...
!     costhetab = roundoff(costhetab,smalldouble)
     ...
     if (.not. file_exists) then
        ...
        write(unitanalysis,"(a)") trim('e2max b2min b2max e2overb2min e2overb2max')
        opened=.true.
     else
     ...
     write(unitanalysis,'(16(ES14.6))')t, e2, b2, jbar, jmin, jmax, &
          e2min, e2max, b2min, b2max, e2overb2min, e2overb2max
